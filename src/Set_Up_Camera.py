...
class Camera_Setup(QDialog):
# Main Window to create the TetrAMM set up modal window
    def __init__(self, camnum, totalcam,parent=None):
    # Class initialiser for the modal
        super().__init__(parent)
        # Style sheet to customise the GUI as needed
        self.setStyleSheet('''
            QLabel {
                font-size: 20px;
                color: #191970;
        ...
        self.initUI(camnum, totalcam) # Initialiser code

    def get_output(self):
    # Function to return the values needed
    # Called externally  
    # Arguments: self    
    # Returns: the pv of the Camera, and arrays containing the qe1 and hdf set up values respectively  
    ...
    # Returns: None
        try:
            number_captures = int(self.seconds_edit.text())*int(self.fps_read.text())
    ...
    # Called when the PV is done being edited
        try:
            fps = caget('%s:CAM:ArrayRate_RBV'%self.pv.text())
            self.fps_read.setText(str(int(fps)))
        except:
            self.fps_read.setText('Invalid')
    ...
    # Returns: the QE1 and HDF5 Set up values
        ...
        fps = self.fps_read.text()
        pvv = self.pv.text()

        # Getting the QE1 Values to return from the EPICS GUI:
        gain = caget ('%s:CAM:Gain'%pvv)
        binning_x = caget ('%s:CAM:BinX'%pvv)
        binning_y = caget ('%s:CAM:BinY'%pvv)
        reg_start_x = caget ('%s:CAM:MinX'%pvv)
        reg_start_y = caget ('%s:CAM:MinY'%pvv)      
        reg_size_x = caget ('%s:CAM:SizeX'%pvv)
        reg_size_y = caget ('%s:CAM:SizeY'%pvv)

        aqu_period = caget ('%s:CAM:AcquirePeriod'%pvv)
        exposure = caget ('%s:CAM:AcquireTime'%pvv)

        # Set the values in their arrays to return to other scripts
        self.pv_text = pvv
        self.hdf = [fp,fn,ff,sec,cap,fps]
        self.qe1 = [gain,binning_x,binning_y,reg_size_x,reg_size_y,reg_start_x,reg_start_y,aqu_period,exposure]

        # Close the GUI
        self.close()

    # NOTE@Claire -- > This has not been written yet, waiting for your fast cam code to be updated 
    def open_fast_cam(self):
    # Function called to run external script to set up the camera, this is called by the button
        pass

    def initUI(self, camnum, totalcam):
    # Initialiser code
        # Layouts
        ...
        pvbtnLayout = QHBoxLayout() # PV button layout
        ...
        pv_frame_buttons = QFrame()
        pv_frame_buttons.setObjectName("PVFrameButtons")
        ...
        self.labelTitle.setText('Camera    (%(a)s/%(b)s)'%{'a': camnum, 'b': totalcam})
        ...
        self.fps_read = QLineEdit()

        self.fps_read.setReadOnly(True)
        ...
        pv_formLayout.insertRow(1, pv_label, self.pv)
        
        self.fastcam_btn = QPushButton(self)
        self.fastcam_btn.setText('FAST CAM')
        self.fastcam_btn.setObjectName('FastcamBtn')
        self.fastcam_btn.setFont(font_medium) 
        self.fastcam_btn.clicked.connect(self.open_fast_cam) # NOTE: This fast cam opening function has not been written yet

        pvbtnLayout.addWidget(self.fastcam_btn)    
        ...
        # Add PV section 
        pv_formLayout.setContentsMargins(0,14,0,0)
        pv_frame.setLayout(pv_formLayout)
        dlgLayout.addWidget(pv_frame)

        pv_frame_buttons.setLayout(pvbtnLayout)
        dlgLayout.addWidget(pv_frame_buttons)
        ...
        formLayout_sec_labels.addWidget(fps_label)        
        ...
        formLayout_sec.addWidget(self.fps_read)
